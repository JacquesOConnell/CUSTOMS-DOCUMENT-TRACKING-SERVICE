...
def handler(event,context):
    ...
    if method=='POST' and path=='/jobs':
        ...
        item={'job_id':job_id,'customer':body['customer'],'contact':body['contact'],'email':body['email'],'filename':body['filename'],'border':body.get('border',''),'origin':body.get('origin',''),'notes':body.get('notes',''),'status':'received','agent_name':'','agent_id':'','s3_key':key,'created_at':now(),'updated_at':now()}
